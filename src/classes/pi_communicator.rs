...
impl PiCommunicator {
    ...
    async fn _send_to_pi(pi_ip: &str, new_state: &States, current_room_pir: &str) {
        ...
        Ok(response) => {
            ...
            println!("State updated successfully for {}", ip);
            } else {
            eprintln!("Failed to update state for {}: {}", ip, response.status());
            }
        }
        Err(err) => {
    ...
    pub async fn send_new_state(res_id: &str, new_state: &States, current_room_pir: &str, db_client: &Client) {
